...
class FranchisorReportService {
  async generateConsolidatedReport(franchisorId, format) {
    const tenantIds =
      await franchisorService.getTenantIdsForFranchisor(franchisorId);
  ...
  generatePdfReport(data) {
    ...
    doc.text(
      `Média de Satisfação: ${data.summary.csat.averageScore.toFixed(1)}`,
    );
    ...
    data.attendantsPerformance.forEach((attendant) => {
      doc
        .fontSize(12)
        .text(`${attendant.name} - NPS: ${attendant.currentNPS.toFixed(1)}`);
    });
    doc.moveDown();
