...
const awardProcessorJob = async () => {
  ...
  try {
    ...
    console.log(
      `Encontradas ${metasComPremio.length} metas com prêmios para processar.`,
    );
    ...
    for (const meta of metasComPremio) {
      ...
      if (!atendente || !tenant) {
        console.warn(
          `Pulando meta ${meta.id} por falta de dados de atendente ou tenant.`,
        );
        continue;
      }
      ...
      switch (period) {
        ...
        default:
          console.warn(
            `Período desconhecido para a meta ${meta.id}: ${period}.`,
          );
          continue;
      }
        ...
        const diasNoPeriodo =
          period === "MENSAL"
            ...
            : period === "SEMANAL"
              ? 7
              : 1;
        const metaProporcional = Math.ceil(
          (responsesGoal / (dias_trabalhados || 22)) * diasNoPeriodo,
        );
...
async function registrarPremiacao(
  meta,
  tipoMeta,
  valorAtingido,
  valorPremio,
  ...
  if (existingAward) {
    console.log(
      `Prêmio de ${tipoMeta} para meta ${meta.id} já registrado neste período.`,
    );
    return;
  }
  ...
  console.log(
    `Prêmio de R$${valorPremio} concedido a ${meta.atendente.name} por atingir meta de ${tipoMeta} com valor ${valorAtingido}.`,
  );
}
