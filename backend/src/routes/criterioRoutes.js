...
router.post(
  ...
  [
    ...
    check("description", "Descrição do critério deve ser uma string")
      .optional()
      .isString(),
  ],
  validate,
...
router.get(
  ...
  criterioController.getAllCriterios,
);
...
router
  ...
  .put(
    [
      check("id", "ID do critério inválido").isUUID().not().isEmpty(),
      check("name", "Nome do critério deve ser uma string não vazia")
        .optional()
        .not()
        .isEmpty(),
      check("description", "Descrição do critério deve ser uma string")
