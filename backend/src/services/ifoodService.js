...
const getGlobalCredentials = () => {
  ...
  if (!clientId || !clientSecret) {
    throw new ApiError(
      500,
      "Configuração iFood incompleta: IFOOD_CLIENT_ID_GLOBAL ou SECRET ausente.",
    );
  }
...
const ifoodService = {
  ...
  async getAuthorizationUrl(tenantId) {
    ...
    try {
      console.log(
        `[iFood Auth] Requesting User Code for tenant ${tenantId} using Client ID: ${clientId}`,
      );

      // Endpoint para fluxo "User Code" (recomendado para integração fácil)
      const response = await ifoodAxios.post(
      ...
      console.error(
        `[iFood Auth] Error getting User Code:`,
      ...
      if (error.response?.status === 403) {
        throw new ApiError(
          403,
          "Acesso negado pelo iFood. Verifique se o Client ID Global está aprovado.",
        );
      }
  ...
  async requestNewAccessToken(tenantId, authCode = null) {
    ...
    try {
      console.log(
        `[iFood Auth] Exchanging code for token for tenant ${tenantId}`,
      );
      ...
      console.error(
        `[iFood Auth] Exchange Token Error:`,
        error.response?.data || error.message,
      );
      throw new ApiError(
        500,
        "Falha ao finalizar conexão. O código pode ter expirado.",
      );
    }
  },
  ...
  async getValidToken(tenantId) {
    ...
    if (
      tenant.ifoodTokenExpiresAt &&
      new Date(tenant.ifoodTokenExpiresAt) > nowPlusBuffer
    ) {
    ...
    if (tenant.ifoodRefreshToken) {
      console.log(
        `[iFood Auth] Token expired for tenant ${tenantId}. Refreshing...`,
      );
  ...
  async refreshAccessToken(tenantId, refreshToken) {
    ...
    try {
      ...
      console.error(
        `[iFood Auth] Refresh Error for tenant ${tenantId}:`,
        error.response?.data || error.message,
      );
  ...
  async getIfoodMerchantData(tenantId) {
    try {
      ...
      const response = await ifoodAxios.get(
        `${IFOOD_MERCHANT_BASE_URL}/merchants`,
        {
      ...
      if (merchant && merchant.id) {
        ...
        console.log(
          `[iFood] Merchant ID ${merchant.id} linked to tenant ${tenantId}`,
        );
        return merchant;
      }
  ...
  async pollEventsForTenant(tenantId) {
    try {
      ...
      const response = await ifoodAxios.get(
        `${IFOOD_EVENTS_BASE_URL}/events:polling`,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
      );
      ...
      if (events.length > 0) {
        console.log(
          `[iFood Job] ${events.length} events for tenant ${tenantId}`,
        );
      ...
      if (error.response?.status !== 204) {
        // 204 No Content é normal
        console.error(
          `[iFood Job] Polling Error Tenant ${tenantId}:`,
          error.message,
        );
      }
    }
  ...
  async handleSingleEvent(tenantId, event) {
    try {
      ...
      console.log(
        `[iFood] Event ${code} for Order ${orderId} (Tenant ${tenantId})`,
      );
  ...
  async processOrderConclusion(tenantId, orderId) {
    try {
      ...
      const order = await deliveryOrderRepository.findByPlatformAndOrderId(
        "iFood",
        orderId,
      );
      ...
      if (order) {
        ...
        console.log(
          `[iFood] Concluded order ${orderId} not found locally. Importing...`,
        );
        await this.processOrder(tenantId, orderId, true); // true = force schedule
      }
    } catch (error) {
      console.error(
        `[iFood] Error processing conclusion for order ${orderId}:`,
        error.message,
      );
    }
  },
  ...
  async acknowledgeEvents(tenantId, eventList, token) {
    try {
      await ifoodAxios.post(
        `${IFOOD_EVENTS_BASE_URL}/events/acknowledgment`,
        eventList,
        {
          headers: { Authorization: `Bearer ${token}` },
        },
  ...
  async processOrder(tenantId, orderId, forceSchedule = false) {
    try {
      ...
      let newOrder = await deliveryOrderRepository.findByPlatformAndOrderId(
        "iFood",
        orderId,
      );
      ...
      if (!newOrder) {
        // Buscar detalhes
        const response = await ifoodAxios.get(
          `${IFOOD_ORDER_BASE_URL}/orders/${orderId}`,
          {
            headers: { Authorization: `Bearer ${token}` },
          },
        );
      ...
      if (forceSchedule) {
        await surveyTriggerService.schedulePostSaleSurvey(
          tenantId,
          newOrder.id,
        );
      }
      ...
      console.error(
        `[iFood] Order Processing Error ${orderId}:`,
        error.message,
      );
    }
  },
