...
async function checkSenderStatuses() {
  console.log(
    `[${JOB_NAME}] Iniciando verificação de status dos disparadores...`,
  );
  ...
  console.log(
    `[${JOB_NAME}] Verificando ${inactiveSenders.length} disparador(es) inativo(s).`,
  );
  ...
  for (const sender of inactiveSenders) {
    try {
      ...
      console.error(
        `[${JOB_NAME}] Erro ao verificar status do disparador ${sender.name}:`,
        error.message,
      );
    }
  }
