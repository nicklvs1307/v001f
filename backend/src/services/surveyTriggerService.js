...
const surveyTriggerService = {
  ...
  async schedulePostSaleSurvey(tenantId, deliveryOrderId) {
    try {
      ...
      console.log(`Pesquisa agendada para o pedido ${deliveryOrderId}. Delay: ${delayMinutes} min. Envio em: ${scheduledAt}`);

    } catch (error) {
      console.error(`Erro ao agendar pesquisa para o pedido ${deliveryOrderId}:`, error);
      await DeliveryOrder.update({ surveyStatus: 'ERROR' }, { where: { id: deliveryOrderId } });
    }
  },
  ...
  async sendSatisfactionSurvey(clientId, tenantId, deliveryOrderId) {
    try {
      ...
      if (!survey) {
        console.warn(`Nenhuma pesquisa ativa encontrada para o tenantId: ${tenantId}.`);
        return; 
      }
      ...
      let messageTemplate = config?.postSaleMessageTemplate || 'Olá {{cliente}}! Agradecemos o seu pedido. Poderia nos dar um feedback rápido para melhorarmos? {{link_pesquisa}}';
