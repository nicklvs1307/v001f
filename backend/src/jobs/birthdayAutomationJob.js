...
const birthdayAutomationJob = cron.schedule('0 9 * * *', async () => { // Executa todo dia às 9h da manhã
    ...
    for (const config of configs) {
      ...
      for (const client of clients) {
        ...
        if (config.birthdayRewardType === 'recompensa') {
          const recompensa = await recompensaRepository.findById(config.birthdayRewardId);
          if (recompensa) {
          ...
          const roleta = await roletaRepository.findById(config.birthdayRewardId);
          if (roleta) {
