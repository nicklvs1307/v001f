...
const getSummary = async (
  ...
  const csatResponses = ratingResponses.filter(
    (r) => r.pergunta.type === "rating_1_5",
  );

  const npsResult = ratingService.calculateNPS(npsResponses);
  const csatResult = ratingService.calculateCSAT(csatResponses);

  const ambassadorsInPeriod = npsResponses.filter(
    (r) => r.ratingValue >= 9,
  ).length;
...
const getMonthSummary = async (
  ...
  npsResponses.forEach((response) => {
    const day = formatInTimeZone(response.createdAt, "yyyy-MM-dd");
    if (!dailyData[day]) {
  ...
  const dailyNps = Object.keys(dailyData).map((day) => {
    ...
    const npsScore =
      totalDaily > 0
        ? (dayData.promoters / totalDaily) * 100 -
          (dayData.detractors / totalDaily) * 100
        : 0;
    ...
    const accumulatedNps =
      accumulatedTotal > 0
        ? (accumulatedPromoters / accumulatedTotal) * 100 -
          (accumulatedDetractors / accumulatedTotal) * 100
        : 0;
  ...
  const responsesByHour = await Resposta.findAll({
    ...
    attributes: [
      [
        sequelize.fn(
          "EXTRACT",
          sequelize.literal(`HOUR FROM "createdAt" AT TIME ZONE '${TIMEZONE}'`),
        ),
        "hour",
      ],
  ...
  const responsesByWeekday = await Resposta.findAll({
    ...
    attributes: [
      [
        sequelize.fn(
          "EXTRACT",
          sequelize.literal(
            `ISODOW FROM "createdAt" AT TIME ZONE '${TIMEZONE}'`,
          ),
        ),
        "weekday",
      ], // 1=Monday, 7=Sunday
    ...
    group: [
      sequelize.fn(
        "EXTRACT",
        sequelize.literal(`ISODOW FROM "createdAt" AT TIME ZONE '${TIMEZONE}'`),
      ),
    ],
    order: [
      [
