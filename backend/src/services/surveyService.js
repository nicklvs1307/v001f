...
const createSurvey = async (surveyData, requestingUser) => {
  ...
  if (recompensaId && roletaId) {
    throw new ApiError(
      400,
      "Uma pesquisa não pode ter uma recompensa e uma roleta ao mesmo tempo.",
    );
  }
...
const deleteSurvey = async (surveyId, requestingUser) => {
  ...
  const deletedRowCount = await surveyRepository.deleteSurvey(
    surveyId,
    tenantId,
  );
  ...
  return surveys.map((survey) => {
    const progress =
      survey.expectedRespondents > 0
        ? (survey.currentRespondents / survey.expectedRespondents) * 100
        : 0;
...
const getSurveyResultsById = async (surveyId, tenantId = null) => {
  ...
  const allRatingResponses = survey.perguntas.reduce((acc, pergunta) => {
    if (pergunta.type.startsWith("rating")) {
      const responsesWithContext = pergunta.respostas.map((r) => ({
        ...r,
        pergunta: { type: pergunta.type, criterio: pergunta.criterio },
      }));
  ...
  const npsResponses = allRatingResponses.filter(
    (r) => r.pergunta.type === "rating_0_10",
  );
  const csatResponses = allRatingResponses.filter(
  ...
  const formattedResults = {
    ...
    questionsResults: survey.perguntas.map((pergunta) => {
      ...
      if (pergunta.type === "multiple_choice" || pergunta.type === "checkbox") {
        ...
        const ratings = pergunta.respostas
  ...
  formattedResults.scoresByCriteria = Object.entries(responsesByCriteria)
