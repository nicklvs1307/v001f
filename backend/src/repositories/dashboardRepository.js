...
const dashboardRepository = {
  getSummary: async (tenantId = null, startDate = null, endDate = null) => {
    ...
    const couponsGenerated = await Cupom.count({
      where: { ...whereClause, createdAt: dateFilter },
    });
    const couponsUsed = await Cupom.count({
      where: { ...whereClause, status: "used", updatedAt: dateFilter },
    });
  ...
  getResponseChart: async (
    ...
    const { dateFilter, daysInRange } = (() => {
      if (startDate && endDate) {
        ...
        return {
          dateFilter: { [Op.gte]: sevenDaysAgo, [Op.lte]: now() },
          daysInRange: 7,
        };
      }
    })();
    ...
    const responseMap = new Map(
      responsesByDay.map((item) => [
        new Date(item.dataValues.date).toISOString().split("T")[0],
      ...
      attributes: [
        ...
        [
          fn(
            "SUM",
            literal(
              `CASE WHEN "respostas"."ratingValue" >= 9 THEN 1 ELSE 0 END`,
            ),
          ),
          "promoters",
        ],
        [
          fn(
            "SUM",
            literal(
              `CASE WHEN "respostas"."ratingValue" BETWEEN 7 AND 8 THEN 1 ELSE 0 END`,
            ),
          ),
          "neutrals",
        ],
        [
          fn(
            "SUM",
            literal(
              `CASE WHEN "respostas"."ratingValue" <= 6 THEN 1 ELSE 0 END`,
            ),
          ),
          "detractors",
        ],
  ...
  getOverallResults: async (tenantId = null) => {
    ...
    if (overallTotalRatingResponses > 0) {
      overallNpsScore =
        (overallPromoters / overallTotalRatingResponses) * 100 -
        (overallDetractors / overallTotalRatingResponses) * 100;
    }
    ...
    allResponses.forEach((response) => {
      ...
      if (pergunta && pergunta.criterio && pergunta.type.startsWith("rating")) {
        ...
        if (!npsCriterioMap.has(criterioName)) {
          npsCriterioMap.set(criterioName, {
            promoters: 0,
            neutrals: 0,
            detractors: 0,
            total: 0,
    ...
    allResponses.forEach((response) => {
      ...
      if (pergunta && pergunta.type.startsWith("rating")) {
        const key = pergunta.criterio
          ? pergunta.criterio.name
          : pergunta.id
            ? `Pergunta ${pergunta.id.substring(0, 4)}`
            : "Pergunta Desconhecida"; // Usar critério ou ID da pergunta
    ...
    for (const attendant of sortedAttendants) {
      ...
      const totalRatingResponses =
        attendant.promoters + attendant.neutrals + attendant.detractors;
      if (totalRatingResponses > 0) {
        attendant.currentNPS =
          (attendant.promoters / totalRatingResponses) * 100 -
          (attendant.detractors / totalRatingResponses) * 100;
      }
    ...
    const responseChartData =
      await dashboardRepository.getResponseChart(tenantId);
    ...
    return {
      ...
      responseChartData,
      feedbacks: await dashboardRepository.getFeedbacks(tenantId),
      conversionChart: await dashboardRepository.getConversionChart(tenantId),
    };
  },
  ...
  getAttendantsPerformanceWithGoals: async (tenantId = null) => {
    ...
    for (const attendantStats of attendantPerformance.values()) {
      ...
      // Calcular NPS atual
      const totalRatingResponses =
        attendantStats.promoters +
        attendantStats.neutrals +
        attendantStats.detractors;
      if (totalRatingResponses > 0) {
        attendantStats.currentNPS =
          (attendantStats.promoters / totalRatingResponses) * 100 -
          (attendantStats.detractors / totalRatingResponses) * 100;
      }
