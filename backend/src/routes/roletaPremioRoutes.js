...
router
  ...
  .post(
    [
      ...
      check("descricao", "Descrição do prêmio deve ser uma string")
        .optional()
        .isString(),
      check(
        "probabilidade",
        "Probabilidade do prêmio é obrigatória e deve ser um número entre 0 e 100",
      )
...
router
  ...
  .put(
    [
      check("id", "ID do prêmio inválido").isUUID().not().isEmpty(),
      check("name", "Nome do prêmio deve ser uma string não vazia")
        .optional()
        .not()
        .isEmpty(),
      check("description", "Descrição do prêmio deve ser uma string")
        .optional()
        .isString(),
      check("chance", "Chance do prêmio deve ser um número entre 0 e 100")
        .optional()
        .isFloat({ min: 0, max: 100 }),
    ],
    validate,
