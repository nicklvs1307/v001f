...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!tenantId) {
      throw new ApiError(
        400,
        "Tenant ID is required to initiate iFood authorization.",
      );
    }
...
router.get(
  ...
  asyncHandler(async (req, res) => {
    ...
    const frontendUrl =
      process.env.FRONTEND_URL || "https://voltaki.towersfy.com";

    if (error) {
      console.error("iFood Callback Error:", error, error_description);
      return res.redirect(
        `${frontendUrl}/dashboard/integracoes?ifood_error=${encodeURIComponent(error_description || error)}`,
      );
    }

    if (!code || !state) {
      return res.redirect(
        `${frontendUrl}/dashboard/integracoes?ifood_error=Missing+code+or+state`,
      );
    }
    ...
    try {
      ...
      res.redirect(
        `${frontendUrl}/dashboard/integracoes?ifood_error=${encodeURIComponent(message)}`,
      );
    }
  }),
);
