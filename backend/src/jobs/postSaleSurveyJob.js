    ...
    cron.schedule(JOB_SCHEDULE, async () => {
      try {
        ...
        if (pendingOrders.length > 0) {
          console.log(`[Post-Sale Survey Job] Encontrados ${pendingOrders.length} pedidos pendentes de pesquisa.`);
          
          for (const order of pendingOrders) {
            try {
              // Chama o método original que faz o envio real
              await surveyTriggerService.sendSatisfactionSurvey(order.clientId, order.tenantId, order.id);
            } catch (innerError) {
              console.error(`[Post-Sale Survey Job] Falha ao processar pedido ${order.id}:`, innerError.message);
