...
const deliveryIntegrationService = {
  processUaiRangoOrder: async function (payload) {
    ...
    try {
      // 1. Find Tenant
      const tenant = await tenantRepository.findByUaiRangoId(
        uairangoEstablishmentId,
      );
      if (!tenant) {
        console.error(
          `[UaiRango Webhook] Tenant não encontrado para uairangoEstablishmentId: ${uairangoEstablishmentId}. Payload: ${JSON.stringify(payload)}`,
        );
        return; // Interrompe o processamento se o tenant não for encontrado
      }

      console.log(
        `[UaiRango Webhook] Tenant encontrado: ${tenant.id} para o pedido ${orderIdPlatform}`,
      );

      // 2. Find or Create Client
      const customerPhone = payload.usuario.tel1.replace(/\D/g, "");
      let client = await clientRepository.findClientByPhone(
        customerPhone,
        tenant.id,
      );
      ...
      if (!client) {
        ...
        console.log(
          `[UaiRango Webhook] Novo cliente criado: ${client.id} para o pedido ${orderIdPlatform}`,
        );
      } else {
        console.log(
          `[UaiRango Webhook] Cliente existente encontrado: ${client.id} para o pedido ${orderIdPlatform}`,
        );
      }

      // 3. Create DeliveryOrder
      const existingOrder =
        await deliveryOrderRepository.findByPlatformAndOrderId(
          "UaiRango",
          orderIdPlatform,
        );
      if (existingOrder) {
        console.log(
          `[UaiRango Webhook] Pedido ${orderIdPlatform} da plataforma UaiRango já existe. Ignorando.`,
        );
        return;
      }
      ...
      console.log(
        `[UaiRango Webhook] DeliveryOrder criado: ${deliveryOrder.id} para o pedido ${orderIdPlatform}`,
      );
      ...
      console.log(
        `[UaiRango Webhook] Pesquisa de satisfação agendada para cliente ${client.id}, pedido ${deliveryOrder.id}.`,
      );
    } catch (error) {
      console.error(
        `[UaiRango Webhook] Erro ao processar pedido ${orderIdPlatform}:`,
        error,
      );
