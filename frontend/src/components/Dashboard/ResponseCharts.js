...
const ResponseCharts = ({ startDate, endDate }) => {
    ...
    const [surveysRespondedChart, setSurveysRespondedChart] = useState([]);
    ...
    useEffect(() => {
        ...
        const fetchChartData = async () => {
            try {
                ...
                const [resChart, surveysChart] = await Promise.all([
                    dashboardService.getResponseChart(params),
                    dashboardService.getSurveysRespondedChart(params)
                ]);

                if (isActive) {
                    setResponseChart(resChart);
                    setSurveysRespondedChart(surveysChart);
                }
            } catch (err) {
                if (isActive) {
