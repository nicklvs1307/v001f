const tenantMiddleware = (req, res, next) => {
  ...
  if (req.user.role === 'Super Admin') {
    tenantId = req.query.tenantId || req.params.tenantId || req.body.tenantId || null;
  } else {
    ...
    if (!tenantId) {
      return res.status(403).json({ message: 'Acesso negado: Tenant ID não encontrado para este usuário.' });
    }
  }
