...
import { Typography, Box, Alert, Paper, CircularProgress, Button, Snackbar } from '@mui/material'; // Added Button, Snackbar
import ContentCopyIcon from '@mui/icons-material/ContentCopy'; // Added Icon
import CameraAltIcon from '@mui/icons-material/CameraAlt'; // Added Icon
...
const CongratulationsComponent = ({ premio, cupom, tenant }) => {
    ...
    const [copySuccess, setCopySuccess] = useState(false); // State for feedback
        ...
        <Box sx={{ background: `linear-gradient(135deg, ${theme.palette.secondary.main} 0%, ${theme.palette.primary.main} 100%)`, minHeight: '100vh', display: 'flex', justifyContent: 'center', alignItems: 'center', p: { xs: 1, sm: 2 }, position: 'relative' }}>
            ...
            <Box sx={{ maxWidth: '800px', width: '100%', backgroundColor: 'white', borderRadius: '20px', boxShadow: '0 15px 30px rgba(0, 0, 0, 0.2)', overflow: 'hidden', textAlign: 'center', position: 'relative', zIndex: 2, margin: { xs: '0 16px', sm: 0 } }}>
                ...
                <Box sx={{ padding: { xs: '20px', sm: '30px', md: '40px' }, position: 'relative', zIndex: 2 }}>
                    ...
                    <Paper elevation={3} sx={{ background: `linear-gradient(45deg, ${theme.palette.background.default}, ${theme.palette.grey[100]})`, borderRadius: '15px', padding: { xs: '20px', sm: '30px' }, margin: '20px 0', border: `2px dashed ${theme.palette.error.main}`, position: 'relative', animation: `${floatAnimation} 3s ease-in-out infinite` }}>
                        ...
                        {/* Área do Código com Botão de Copiar */}
