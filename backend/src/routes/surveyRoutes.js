...
const surveyQuestionsValidation = [
  ...
  check(
    "questions.*.text",
    "Texto da pergunta é obrigatório e deve ser uma string.",
  )
    .isString()
    .notEmpty(),
  check(
    "questions.*.type",
    "Tipo da pergunta é obrigatório e deve ser válido.",
  ).isIn([
    "rating_1_5",
    "rating_0_10",
    "free_text",
...
router.get(
  "/qrcode",
  authorize("surveys:read"),
  surveyController.generateSurveyQrCode,
);
...
router.post(
  ...
  [
    ...
    check("startDate", "Data de início inválida")
      ...
      .isISO8601()
      .toDate(),
    check("endDate", "Data de término inválida")
      .optional({ nullable: true, checkFalsy: true })
      .isISO8601()
      .toDate(),
    ...
    check("isOpen", "O campo 'isOpen' deve ser um booleano")
      .optional()
      .isBoolean(),
...
router
  ...
  .put(
    authorize("surveys:update"),
    [
      check("title", "Título da pesquisa deve ter no mínimo 3 caracteres")
        .optional()
        .isLength({ min: 3 }),
