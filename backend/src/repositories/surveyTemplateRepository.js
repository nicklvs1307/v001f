const { SurveyTemplate, Pesquisa, Pergunta, Tenant, Resposta } = require("../../models");
...
const surveyTemplateRepository = {
  createTemplate: async (templateData, requestingUser) => {
    ...
    if (isSystemTemplate && requestingUser.role !== 'Super Admin') {
      throw new ApiError(403, "Você não tem permissão para criar um template de sistema.");
    }
  ...
  updateTemplate: async (id, templateData, requestingUser) => {
    ...
    if (template.isSystemTemplate && requestingUser.role !== 'Super Admin') {
      throw new ApiError(403, "Você não tem permissão para editar um template de sistema.");
    }

    // Admins só podem editar templates do seu próprio tenant
    if (!template.isSystemTemplate && template.tenantId !== requestingUser.tenantId) {
      throw new ApiError(403, "Você não tem permissão para editar este template.");
    }
  ...
  deleteTemplate: async (id, requestingUser) => {
    ...
    if (template.isSystemTemplate && requestingUser.role !== 'Super Admin') {
      throw new ApiError(403, "Você não tem permissão para deletar um template de sistema.");
    }

    // Admins só podem deletar templates do seu próprio tenant
    if (!template.isSystemTemplate && template.tenantId !== requestingUser.tenantId) {
      throw new ApiError(403, "Você não tem permissão para deletar este template.");
    }
  ...
  createSurveyFromTemplate: async (templateId, newSurveyData, requestingUser) => {
    ...
    try {
      ...
      if (!template.isSystemTemplate && template.tenantId !== requestingUser.tenantId) {
        throw new ApiError(403, "Você não tem permissão para usar este template.");
      }

      const { title, description, atendenteId, isOpen, startDate, endDate, status, expectedRespondents, askForAttendant, dueDate } = newSurveyData;
      ...
      const newSurvey = await Pesquisa.create({
        ...
        askForAttendant: askForAttendant !== undefined ? askForAttendant : false,
  ...
  overwriteSurveyWithTemplate: async (surveyId, templateId, requestingUser) => {
    ...
    try {
      ...
      if (requestingUser.role !== "Super Admin" && survey.tenantId !== requestingUser.tenantId) {
        throw new ApiError(403, "Você não tem permissão para sobrescrever esta pesquisa.");
      }
      ...
      await survey.update({
        ...
        askForAttendant: template.askForAttendant !== undefined ? template.askForAttendant : false,
