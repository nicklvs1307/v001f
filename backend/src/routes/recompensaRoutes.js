...
router.post(
  ...
  [
    ...
    check("description", "Descrição da recompensa deve ser uma string").optional().isString(),
    check("pointsRequired", "Pontos necessários são obrigatórios e devem ser um número inteiro maior que 0").isInt({ gt: 0 }).not().isEmpty(),
  ],
  validate,
...
router
  ...
  .put(
    [
      check("id", "ID da recompensa inválido").isUUID().not().isEmpty(),
      check("name", "Nome da recompensa deve ser uma string não vazia").optional().not().isEmpty(),
      check("description", "Descrição da recompensa deve ser uma string").optional().isString(),
      check("pointsRequired", "Pontos necessários devem ser um número inteiro maior que 0").optional().isInt({ gt: 0 }),
    ],
    validate,
