...
const deliveryMuchService = {
  ...
  async getValidToken(tenantId) {
    ...
    if (
      tenant.deliveryMuchToken &&
      tenant.deliveryMuchTokenExpiresAt &&
      new Date(tenant.deliveryMuchTokenExpiresAt) > nowPlusBuffer
    ) {
  ...
  async authenticate(tenant) {
    if (
      !tenant.deliveryMuchClientId ||
      !tenant.deliveryMuchClientSecret ||
      !tenant.deliveryMuchUsername ||
      !tenant.deliveryMuchPassword
    ) {
      throw new Error(
        `Credenciais Delivery Much incompletas para o Tenant ${tenant.id}`,
      );
    }
    ...
    try {
      ...
      console.error(
        `[Delivery Much Auth] Error for tenant ${tenant.id}:`,
        error.response?.data || error.message,
      );
      throw new ApiError(
        401,
        "Falha na autenticação com Delivery Much. Verifique as credenciais.",
      );
    }
  },
  ...
  async fetchOrdersForTenant(tenant) {
    try {
      ...
      if (orders.length > 0) {
        console.log(
          `[Delivery Much Job] ${orders.length} orders found for tenant ${tenant.id}`,
        );
      ...
      console.error(
        `[Delivery Much Job] Error fetching orders for tenant ${tenant.id}:`,
        error.response?.data || error.message,
      );
    }
  },
  ...
  async processOrder(tenantId, order) {
    try {
      ...
      const exists = await deliveryOrderRepository.findByPlatformAndOrderId(
        "DeliveryMuch",
        orderIdPlatform,
      );
      if (exists) return;
      ...
      if (!customerPhone) {
        console.log(
          `[Delivery Much] Pedido ${orderIdPlatform} sem telefone. Ignorando automação.`,
        );
        return;
      }
